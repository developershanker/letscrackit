...
export const AddDetails: React.FC = () => {
  ...
  const lastEntry = physicalData?.[0];

  const [weight, setWeight] = useState(lastEntry?.weight ? String(lastEntry.weight) : '');
  const [height, setHeight] = useState(lastEntry?.height ? String(lastEntry.height) : '');
